...
# 4t. O `telegramId` ganha caminho de edição (18/08/2026) — B-D120 e B-D121

Ele foi olhar o Perfil em produção, o campo não estava lá, e pediu que estivesse. **Isto supera parcialmente o B-D105** — ver R10, abaixo — que tinha assumido a falta de edição como custo pequeno. O resto de B-D105 (validação frouxa, `NULLIF` no vazio) continua valendo; só a parte "sem caminho de edição" caiu.

| # | Decisão | Motivo resumido |
|---|---|---|
| B-D120 | **`PUT /api/perfil/telegram`, corpo `{ "telegramId": "..." }`, resposta é o mapa de perfil inteiro** — mesma forma de `PUT /api/perfil/nome`. **A validação é a mesma do cadastro** (`@Size(max = 64)`, `@Pattern` aceitando id numérico e `@usuario`), e **vazio grava `NULL`**, pelo mesmo motivo do `NULLIF(btrim(...), '')` da V18: `ux_usuario_telegram` é índice parcial, e string vazia seria valor real para ele. Duas bordas com regras diferentes para o mesmo campo seria divergência, não flexibilidade. **Nenhuma migração e nenhuma função `SECURITY DEFINER`**: a V8 já revogou só o `SELECT` da coluna e manteve a escrita concedida, `pol_usuario_escrita` (V15) já casa `id = app_usuario_id()`, e no Perfil há identidade na sessão — o impasse do cadastro (sem sessão, valor tem que entrar na inserção) não existe aqui. Mesmo raciocínio do B-D73, que recusou função nova para a troca de senha pelo mesmo motivo. **Nota de 18/08/2026:** a afirmação valia para **este endpoint** — nenhuma migração nasceu dele. A V23 chegou depois, pela mesma entrega, mas por outro motivo: o `qa-adversarial` achou I-41/I-42 no índice que já existia desde a V1/V18, não em nada que este endpoint tenha introduzido. Ver a Nota do B-D121, abaixo, e o registro completo em `inconsistencias.md` |
| B-D121 | **Não há verificação de posse do `telegramId`.** Quem digita o `@usuario` ou o id de outra pessoa simplesmente não é reconhecido pelo bot quando ele existir — não há confirmação, não há "código enviado". É escolha, não pendência: o índice único (`ux_usuario_telegram`) já impede duas contas apontando para o mesmo destino, e o dano de errar é não funcionar, não vazar nada de terceiro. Custo em voz alta, no estilo do B-D88: quem digitar um Telegram alheio por engano não recebe aviso nenhum de que aquele valor pertence à conta errada — só descobre quando o bot não reconhecer ninguém do outro lado. **Nota de 18/08/2026:** ao escrever esta linha, o índice ainda comparava texto literal — `abner`, `ABNER` e `@abner` passavam como três destinos diferentes (I-41). A V23 corrigiu o índice para a forma normalizada. Isto **torna a premissa desta decisão verdadeira**; não a revoga. A decisão de não verificar posse continua sendo a mesma, agora apoiada num índice que de fato cumpre o que dele se dizia. **Custo adicional, mesmo molde do B-D81:** o 409 é oráculo de enumeração — confirma que aquele `@usuario` tem conta no RaspyBank, hoje desde a V18 no cadastro, não algo novo desta entrega. Aceito pelo mesmo motivo do e-mail: dizer a verdade custa uma enumeração lenta, e mentir custaria alguém não descobrir o Telegram errado nunca |

# 5. Revisões registradas (R1–R10, sessão de requisitos)
...
### R10 — B-D105 perde a parte "sem caminho de edição" *(18/08/2026)*
*Antes:* B-D105 registrava, como custo assumido, que o `telegramId` do cadastro não tinha como se corrigir depois — "pequeno porque o campo não é o login". *Depois:* B-D120 abre `PUT /api/perfil/telegram`, com a mesma validação frouxa e o mesmo `NULL` no vazio. *Motivo:* não foi reavaliação técnica — ele foi olhar o Perfil em produção, o campo não estava lá, e pediu que estivesse. O resto de B-D105 (validação frouxa de propósito, coluna desde a V1, impasse de sessão no cadastro) continua valendo; só a ausência de edição deixou de ser verdade.
...
| V23 | **Telegram é um destino só** (I-41/I-42, 18/08/2026): `ux_usuario_telegram` recriado — mesmo nome, de propósito, porque `TratadorGlobalDeErros` casa a string do erro para devolver 409 — sobre `lower(regexp_replace(btrim(telegram_id), '^@', ''))`, mantendo a parcialidade `WHERE telegram_id IS NOT NULL`; `ck_usuario_telegram_identificavel` com a mesma expressão, cobrindo `''`, espaços e `'@'` sozinho numa regra só. **Sem função `SECURITY DEFINER` nova** — a expressão vai inline nas duas, seguindo o precedente de `ux_usuario_email`/`lower(email)`; índice sobre função própria envelhece em silêncio, porque `CREATE OR REPLACE` não reconstrói o índice. Dado existente: `''`/`'   '` viram `NULL` com `RAISE NOTICE`; `'@'` sozinho e colisão real **falham a migração** com relatório, em vez de escolher vencedor. Verificada por `TelegramNoPerfilTest` (I-41/I-42) | ✔ |

**Por que não há V21 nesta tabela.** O extrato completo (V22) e a correção do Telegram (V23) foram numerados em branches paralelas, cada uma olhando só a sua: as duas escolheram `V21`. O relatório cedeu o número primeiro, e depois a fatia do Telegram cedeu também — o Flyway não aceita versão menor chegando depois da maior, e a V22 já estava na `main`. Daí V22 e V23 sem nenhuma V21 entre elas. **O número não se perdeu por descuido: ele nunca chegou a ser aplicado em lugar nenhum**, então renumerar ainda era renomear arquivo. A lição está no I-24, junto da terceira pretendente ao mesmo número — que segue sem migração escrita.
